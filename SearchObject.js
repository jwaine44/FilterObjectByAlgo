...
// Filters the given items based on the search criteria using a startsWith
let expected = [];
let upperCase = searchFor.toUpperCase()
    for(let i = 0; i < items.length; i++){
        obj = items[i]
        let comparisonKey = obj[searchBy].toUpperCase()
        if(comparisonKey.startsWith(upperCase))
            expected.push(obj)
    }
    return expected;
    }
...
if(searchMethod === "startsWith"){
    ...
    item[searchBy].startsWith(searchFor)
    )
    return filteredList
  }
 else if(searchMethod === "endsWith"){
    const filteredList = items.filter(item =>
    item[searchBy].endsWith(searchFor)
    )
    return filteredList
  }
  else{
    const filteredList = items.filter(item =>
    item[searchBy].toUpperCase().includes(searchFor.toUpperCase())
    )
    return filteredList
  }
}
...
function functionalFilterByKey(items, searchFor, searchBy, searchMethod = "startsWith") {
    return items.filter(item =>
        item[searchBy].toUpperCase()[searchMethod](searchFor.toUpperCase())
    )
}
